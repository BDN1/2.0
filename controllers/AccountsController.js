...
module.exports = class AccountsController extends require("./Controller") {
  ...
  register(user) {
    ...
    if (newUser) {
      ...
      } else this.HttpContext.response.conflict();
    } else this.HttpContext.response.unprocessable();
  }
  ...
  modify(user) {
    if (this.writeAuthorization()) {
      ...
      if (this.repository != null) {
        ...
        } else if (updateResult == this.repository.updateResult.conflict)
          this.HttpContext.response.conflict();
        else if (updateResult == this.repository.updateResult.notfound)
          this.HttpContext.response.notFound();
        // this.repository.updateResult.invalid
        else this.HttpContext.response.unprocessable();
  ...
  remove(id) {
    ...
    let repo = new ImagesRepository();
    let userRepo = new usersRepository();
    let images = repo.getAll();
    let indexToDelete = [];
    let index = 0;
    ...
    repo.removeByIndex(indexToDelete);
    super.remove(id);
    userRepo.remove(id);
  }
};
